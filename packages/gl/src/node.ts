...
import { Mat44, setS44 } from '@thi.ng/vectors/mat44';
import { Mat33, setS33 } from '@thi.ng/vectors/mat33';

function fromRotationTranslationScale(out: Mat44, q: quat, v: Vec3, s: Vec3) {
...
function normalFromMat4(out: Mat33, a: Mat44) {
...
export class Node {
  ...
  protected _model: Mat44 = Mat44.identity();
  protected _modelDirty: boolean = true;

  modelView: Mat44 = Mat44.identity();
  normal: Mat33 = new Mat33();

  parent: Node;
  children: Array<Node> = [];

  set position(val: Vec3) { this._position.set(val); this._modelDirty = true; }
  ...
  set scale(val: Vec3) { this._scale.set(val); this._modelDirty = true; }
  ...
  updateMatrices(view: Mat44) {
    this.modelView = view.copy().mul(this.model);
